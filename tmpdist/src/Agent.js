...
export class Agent {
     ...
     */
    async _ensureRemoteId() {
        if (this._config.environment.type !== 'remote')
            return;
     ...
     */
    static async create(jsonConfig, callbacks) {
    ...
    constructor(config, callbacks) {
        ...
        if (config.environment.type === 'remote' && !callbacks?.getRemoteId) {
            console.warn('Remote environment specified without getRemoteId callback. ' +
    ...
    async processQuery(query, model, sessionState) {
        ...
        if (!chosenModel) {
            throw new Error('Model must be supplied either in processQuery() or as defaultModel in config');
        }
    ...
    async runConversation(initialQuery, model) {
        ...
        const chosenModel = model ?? this._config.defaultModel;
        if (!chosenModel) {
            throw new Error('Model must be supplied either in runConversation() or as defaultModel in config');
        }
