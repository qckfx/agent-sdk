...
import { isTextBlock } from '../types/anthropic.js';
/** @internal */
export class FsmDriver {
     ...
     */
    async run(query, sessionState, model) {
        // Initialize tracking
        const toolResults = [];
        let finalAssistant;
        let currentToolCall;
        ...
        while (!isTerminal(this.state)) {
            ...
            if (abortSignal?.aborted) {
                ...
                if (this.state.type === 'WAITING_FOR_TOOL_RESULT' &&
                    currentToolCall &&
            ...
            switch (this.state.type) {
                case 'WAITING_FOR_MODEL': {
                    ...
                    const toolCallChat = await modelClient.getToolCall(query, model, toolRegistry.getToolDescriptions(), sessionState, abortSignal ? { signal: abortSignal } : undefined);
                ...
                case 'WAITING_FOR_TOOL_RESULT': {
                    ...
                    try {
                        ...
                        await withToolCall(currentToolCall, sessionState, toolResults, (ctx) => toolRegistry.executeToolWithCallbacks(currentToolCall.toolId, currentToolCall.toolUseId, currentToolCall.args, ctx), {
                ...
                case 'WAITING_FOR_MODEL_FINAL': {
                    ...
                    const finalToolCallChat = await modelClient.getToolCall(`Based on the result of the previous tool execution, what should I do next to answer: ${query}`, model, toolRegistry.getToolDescriptions(), sessionState, abortSignal ? { signal: abortSignal } : undefined);
                    // Check for abort after model call
                    if (abortSignal?.aborted) {
                        console.log(`[FsmDriver] Detected abort after model call, requesting abort`);
                        this.dispatch({ type: 'ABORT_REQUESTED' });
        ...
        if (finalAssistant && finalAssistant.content && finalAssistant.content.length > 0) {
            const textBlocks = finalAssistant.content.filter(isTextBlock);
...
//# sourceMappingURL=FsmDriver.js.map