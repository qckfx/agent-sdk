...
function getSessionId(sessionState) {
    ...
    return sessionState.id || 'unknown-session';
}
/**
...
export function createModelClient(config) {
    ...
    const promptManager = config.promptManager || createDefaultPromptManager();
    return {
        /**
         ...
         */
        formatToolsForClaude(toolDescriptions) {
         ...
         */
        async getToolCall(query, model, toolDescriptions, sessionState, options) {
         ...
         */
        async generateResponse(query, model, toolDescriptions, sessionState, options) {
            // Early abort check
            if (options?.signal?.aborted) {
