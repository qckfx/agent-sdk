...
function convertMessageToOpenAI(msg) {
    ...
    if (role === 'assistant') {
        ...
        if (Array.isArray(msg.content) && msg.content.length > 0 && msg.content[0].type === 'tool_use') {
            ...
            }; // Cast to satisfy structural match
        }
    }
    if (role === 'user') {
        ...
        let tool_call_id;
        if (Array.isArray(msg.content) && msg.content.length > 0 && msg.content[0].tool_use_id) {
    ...
    return {
        ...
        content: collapseText(msg.content),
    };
}
/**
 * Convert Anthropic API params to an OpenAI chat/completions request body.
 */
function convertAnthropicRequestToOpenAI(apiParams) {
    const messages = [];
    // Handle system prompt if provided
    if ('system' in apiParams && apiParams.system) {
        if (typeof apiParams.system === 'string') {
            messages.push({ role: 'system', content: apiParams.system });
        }
        else if (Array.isArray(apiParams.system)) {
            const blocks = apiParams.system;
    ...
    return {
        ...
        messages,
        tools: convertToolsToOpenAI(apiParams.tools),
        // The OpenAI type expects ChatCompletionToolChoiceOption or undefined.
        tool_choice: apiParams.tool_choice?.type,
    };
}
/**
 ...
 */
function getOpenAIClient(sessionLlmApiKey) {
...
async function callOpenAI(requestBody, logger, llmApiKey) {
 ...
 */
function convertOpenAIResponseToAnthropic(openaiResp) {
    ...
    }; // Cast to satisfy structural typing
...
function createModelListFetcher() {
     ...
     */
    async function fetchModelList(llmKey) {
     ...
     */
    async function getAvailableModels(llmKey) {
        try {
 ...
 */
async function withRetryAndBackoff(fn, maxRetries = 5, initialDelay = 1000, maxDelay = 30000, logger) {
    let retries = 0;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (logger) {
                logger.warn(`Rate limit hit, retrying in ${Math.round(delay)}ms (attempt ${retries}/${maxRetries})`, LogCategory.MODEL);
            }
            else {
    ...
    const provider = async (prompt) => {
        try {
            ...
            try {
                ...
                const info = list.find((m) => m.model_name === modelToUse);
            ...
            if (prompt.sessionState && conversationHistory.length > 2) {
                try {
                    ...
                    if (prompt.tools) {
                        tokenCountParams.tools = prompt.tools;
                    }
                    let tokenCount = null;
                    ...
                    if (tokenCount && tokenCount.input_tokens > TARGET_TOKEN_LIMIT) {
                        logger?.warn(`Token count (${tokenCount.input_tokens}) exceeds target limit (${TARGET_TOKEN_LIMIT}). Compressing conversation.`, LogCategory.MODEL, {
                            tokenCount: tokenCount.input_tokens,
                        ...
                        logger?.info(`Compressed conversation history to ${prompt.sessionState.contextWindow.getLength()} messages before API call.`, LogCategory.MODEL);
                    }
                }
                catch (error) {
            ...
            let systemContent = prompt.systemMessage || '';
            ...
            if (shouldUseCache &&
                ...
                modifiedMessages = JSON.parse(JSON.stringify(modifiedMessages));
                ...
                for (let i = modifiedMessages.length - 1; i >= 0; i--) {
                    if (modifiedMessages[i].role === 'user') {
                        ...
                        if (Array.isArray(content) && content.length > 0) {
                            ...
                            const contentWithCache = content[lastContentIndex];
            ...
            if (prompt.systemMessages && prompt.systemMessages.length > 0) {
                ...
                apiParams.system = systemContent;
            }
            else {
            ...
            if (modifiedTools) {
                apiParams.tools = modifiedTools;
            }
            // Add tool_choice if provided
            if (prompt.tool_choice) {
                apiParams.tool_choice = prompt.tool_choice;
            }
            try {
                // ------------------------------------------------------------------
                ...
                const openaiResponse = await withRetryAndBackoff(() => callOpenAI(openaiRequest, logger, llmApiKey), 5, 1000, 30000, logger);
                // ------------------------------------------------------------------
                ...
                logger?.debug('Anthropic API response', LogCategory.MODEL, {
                    ...
                    contentTypes: messageResponse.content?.map((c) => c.type)
                });
                ...
                const isTokenLimitError = apiError.status === 400 &&
                ...
                if (isTokenLimitError && prompt.sessionState && prompt.sessionState.contextWindow.getLength() > 0) {
                    logger?.warn(`Token limit exceeded ${apiError.message ? `(${apiError.message})` : ''}. Attempting to compress conversation history.`, LogCategory.MODEL);
                    // Use token manager to compress conversation to target limit (half of max)
                    // Ensure we pass the logger that matches the expected interface
                    tokenManager.manageConversationSize(prompt.sessionState, TARGET_TOKEN_LIMIT, logger);
                    logger?.info(`Compressed conversation history to ${prompt.sessionState.contextWindow.getLength()} messages. Retrying API call.`, LogCategory.MODEL);
                    ...
                    const openaiRetryResponse = await withRetryAndBackoff(() => callOpenAI(openaiRetryRequest, logger, llmApiKey), 3, // fewer retries for the second attempt
                    1000, 30000, logger);
