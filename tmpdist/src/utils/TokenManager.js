 ...
 */
const trackTokenUsage = (response, sessionState) => {
 ...
 */
const manageConversationSize = (sessionState, maxTokens = 60000, logger) => {
            ...
            if (toolUse) {
                ...
                for (let j = i + 1; j < sessionState.contextWindow.getLength(); j++) {
                    ...
                    if (resultMessage.role === 'user' &&
                        ...
                        resultMessage.content.some((c) => c.type === 'tool_result' && c.tool_use_id === toolUseId)) {
    ...
    if (tokensRemoved < tokensToRemove) {
        ...
        for (let i = 0; i < Math.min(safeCutoff, lastUserMessageIndex); i++) {
            ...
            const containsToolUse = Array.isArray(message.content) && message.content.some((c) => c.type === 'tool_use');
    ...
    if (tokensRemoved < tokensToRemove) {
        ...
        for (let i = safeCutoff; i < sessionState.contextWindow.getLength(); i++) {
            ...
            const isToolUse = Array.isArray(msg.content) && msg.content.some((c) => c.type === 'tool_use');
    ...
    sessionState.contextWindow.setMessages(sessionState.contextWindow.getMessages().filter((_, idx) => !removedIndices.has(idx)));
    // Update token tracking
    const newTokensByMessage = [];
