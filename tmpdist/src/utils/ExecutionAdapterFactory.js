 ...
 */
export async function createExecutionAdapter(options) {
    ...
    try {
        if (type === 'docker') {
            ...
            concreteAdapter = new CheckpointingExecutionAdapter(dockerAdapter, pwd, options.sessionId);
        ...
        if (type === 'e2b') {
            ...
            // Create concrete adapter
            let concreteAdapter = e2bAdapter;
            const res = await e2bAdapter.executeCommand('get-pwd', 'pwd');
            const pwd = res.stdout.trim();
            concreteAdapter = new CheckpointingExecutionAdapter(e2bAdapter, pwd, options.sessionId);
        ...
        logger?.error(`Failed to create ${type} execution adapter: ${error.message}`, error, LogCategory.SYSTEM);
        // If auto fallback is disabled, rethrow the error
        if (!autoFallback) {
            throw error;
        }
        // Log warning about fallback
        logger?.warn(`Falling back to local execution: ${fallbackReason || error.message}`, LogCategory.SYSTEM);
    }
    // Fall back to local execution
    logger?.info('Creating local execution adapter', LogCategory.SYSTEM);
    // Create concrete adapter
    const localAdapter = new LocalExecutionAdapter({ logger });
    let concreteAdapter = localAdapter;
    const res = await localAdapter.executeCommand('get-pwd', 'pwd');
    const pwd = res.stdout.trim();
    // Wrap with checkpointing 
    concreteAdapter = new CheckpointingExecutionAdapter(localAdapter, pwd, options.sessionId);
...
//# sourceMappingURL=ExecutionAdapterFactory.js.map