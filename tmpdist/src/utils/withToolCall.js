 ...
 */
export async function withToolCall(toolCall, sessionState, toolResults, exec, context) {
    ...
    try {
        ...
        if (toolCall.toolUseId) {
            ...
            const firstBlock = Array.isArray(lastMsg?.anthropic.content)
                ? lastMsg.anthropic.content[0]
                : undefined;
            const stillHasToolUse = firstBlock?.type === 'tool_use' && firstBlock.id === toolCall.toolUseId;
            if (stillHasToolUse) {
                ...
                console.warn(`[withToolCall] Skipping tool_result for toolUseId=${toolCall.toolUseId} because context was rolled back`);
...
//# sourceMappingURL=withToolCall.js.map