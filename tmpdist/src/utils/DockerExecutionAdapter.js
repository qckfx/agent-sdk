...
export class DockerExecutionAdapter {
     ...
     */
    constructor(containerManager, options) {
        ...
        this.initializeContainer().then(() => {
            ...
            this.logger?.error(`Background Docker initialization failed: ${error.message}`, error, LogCategory.SYSTEM);
        });
    }
    /**
    ...
    initializeContainer() {
        ...
        return this.containerManager.ensureContainer()
            ...
            .catch(error => {
            this.logger?.error(`Error initializing Docker container: ${error.message}`, error, LogCategory.SYSTEM);
            // Emit error status
    ...
    async executeCommand(executionId, command, workingDir) {
        try {
            ...
            this.logger?.error(`Error executing command in container: ${error.message}`, error, LogCategory.TOOLS);
            return {
                stdout: '',
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const result = await this.executeCommand(executionId, `binary-replace.sh "${originalFile}" "${searchFile}" "${replaceFile}" "${newFile}" 2>&1`);
                    ...
                    console.error(`[ERROR] Failed to execute binary-replace.sh: ${execError.message}`);
                    // We'll set error message but continue to handle it in the next section
                    scriptError = execError.message;
                }
    ...
    async glob(executionId, pattern, _options) {
        try {
            ...
            this.logger?.error(`Error in glob: ${error.message}`, error, 'tools');
            return [];
        }
    }
    /**
        ...
        try {
            ...
            if (details) {
                ...
                for (let i = 1; i < lines.length; i++) {
                    ...
                    if (parts.length < 7)
                        continue; // Need at least enough parts for a valid entry
     ...
     */
    toContainerPath(hostPath, containerInfo) {
        ...
        if (absolutePath.startsWith(containerInfo.projectPath)) {
            return path.join(containerInfo.workspacePath, path.relative(containerInfo.projectPath, absolutePath));
        }
     ...
     */
    toHostPath(containerPath, containerInfo) {
        if (containerPath.startsWith(containerInfo.workspacePath)) {
            return path.join(containerInfo.projectPath, containerPath.substring(containerInfo.workspacePath.length + 1));
        }
     ...
     */
    formatPathForDisplay(absolutePath, containerInfo) {
     ...
     */
    isPathWithinWorkingDir(filepath, containerInfo) {
    ...
    async generateDirectoryMap(rootPath, maxDepth = 10) {
        try {
            ...
            console.error(`DockerExecutionAdapter: Error generating directory map: ${error.message}`);
  ...
  - (Error mapping directory structure)
    ...
    async getGitRepositoryInfo() {
        try {
            ...
            const scriptLines = [`cd "${workingDir}"`];
            ...
            const cachedExecutor = async (command) => {
...
//# sourceMappingURL=DockerExecutionAdapter.js.map