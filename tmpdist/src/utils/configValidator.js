...
// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-explicit-any
const AjvConstructor = AjvNs.default ?? AjvNs;
...
export class ConfigValidationError extends Error {
    constructor(validationErrors, extraMsg) {
...
export function validateConfig(config) {
    ...
    const hasSubAgentTools = Array.isArray(cfgAny.tools)
        && cfgAny.tools.some((t) => typeof t === 'object' && t !== null && 'configFile' in t);
    if (hasSubAgentTools && !cfgAny.experimentalFeatures?.subAgents) {
        throw new ConfigValidationError([], 'Sub-agent tools are still experimental. Add "experimentalFeatures": { "subAgents": true } to your config to enable them.');
    }
    // ---------------------------------------------------------------------
    ...
    if (usesPromptFile && !cfgAny.experimentalFeatures?.promptFiles) {
        throw new ConfigValidationError([], 'Using an external prompt file is still experimental. Add "experimentalFeatures": { "promptFiles": true } to your config to enable it.');
    }
    // ---------------------------------------------------------------------
    ...
    if (usesLocalEnv && !cfgAny.experimentalFeatures?.localEnvironment) {
        throw new ConfigValidationError([], 'Running in the host environment (type = "local") is still experimental. Add "experimentalFeatures": { "localEnvironment": true } to your config to enable it.');
    }
    return config;
}
