...
export class Logger {
     ...
     */
    shouldLog(messageLevel, category) {
        // Check if silent or level is SILENT
        if (this.silent || this.level === LogLevel.SILENT)
            return false;
    ...
    debug(message, categoryOrArg, ...args) {
        let category = LogCategory.SYSTEM;
        let logArgs = [];
        if (categoryOrArg !== undefined) {
            if (Object.values(LogCategory).includes(categoryOrArg)) {
                category = categoryOrArg;
                logArgs = args;
            }
            else {
    ...
    info(message, categoryOrArg, ...args) {
        let category = LogCategory.SYSTEM;
        let logArgs = [];
        if (categoryOrArg !== undefined) {
            if (Object.values(LogCategory).includes(categoryOrArg)) {
    ...
    warn(message, categoryOrArg, ...args) {
        let category = LogCategory.SYSTEM;
        let logArgs = [];
        if (categoryOrArg !== undefined) {
            if (Object.values(LogCategory).includes(categoryOrArg)) {
    ...
    error(message, errorOrCategory, categoryOrData) {
        let error;
        let category = LogCategory.SYSTEM;
        let data;
        if (errorOrCategory !== undefined) {
            if (Object.values(LogCategory).includes(errorOrCategory) && categoryOrData === undefined) {
                category = errorOrCategory;
            }
            else {
                error = errorOrCategory;
                if (categoryOrData !== undefined) {
                    if (Object.values(LogCategory).includes(categoryOrData)) {
