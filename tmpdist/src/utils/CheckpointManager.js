 ...
 */
const getShadowDir = (repoRoot, sessionId) => {
 ...
 */
const gitCommand = (shadowDir, repoRoot, cmd) => {
 ...
 */
const ensureShadowDirIgnored = (repoRoot) => {
    try {
 ...
 */
const makeExcludeFile = async (repoRoot, shadowDir, adapter) => {
 ...
 */
export async function init(repoRoot, sessionId, adapter) {
 ...
 */
export async function snapshot(meta, adapter, repoRoot) {
    ...
    const readRes = await adapter.readFile(executionId, bundlePath, 50 * 1024 * 1024, // 50 MB
    undefined, undefined, 'base64');
    if (!readRes || !readRes.success) {
        throw new Error(`Snapshot failed: unable to read bundle file (${readRes?.error || 'unknown error'})`);
    }
    const bundleBuffer = Buffer.from(readRes.content, 'base64');
 ...
 */
export async function restore(sessionId, adapter, repoRoot, toolExecutionId) {
    ...
    const revParseRes = await adapter.executeCommand(executionId, gitCommand(shadowDir, repoRoot, `rev-parse ${targetRef}`));
    ...
    const checkoutRes = await adapter.executeCommand(executionId, gitCommand(shadowDir, repoRoot, `checkout -f ${resolvedSha}`));
