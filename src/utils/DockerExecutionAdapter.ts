...
export class DockerExecutionAdapter implements ExecutionAdapter {
  ...
  constructor(
    ...
    this.initializeContainer()
      ...
      .catch(error => {
        this.logger?.error(
          `Background Docker initialization failed: ${(error as Error).message}`,
          error,
          LogCategory.SYSTEM,
        );
      });
  ...
  public initializeContainer(): Promise<ContainerInfo | null> {
    ...
    return this.containerManager
      ...
      .catch(error => {
        this.logger?.error(
          `Error initializing Docker container: ${(error as Error).message}`,
          error,
          LogCategory.SYSTEM,
        );
  ...
  async executeCommand(
    ...
    try {
      ...
      try {
        ...
        if (
          (error as Error).message.includes('container not running') ||
          ...
          const retryResult = await this.containerManager.executeCommand(
            command,
            containerWorkingDir,
          );
          return retryResult;
      ...
      this.logger?.error(
        `Error executing command in container: ${(error as Error).message}`,
        error,
        LogCategory.TOOLS,
      );
      return {
  ...
  async readFile(
    ...
    try {
      ...
      if (lineOffset > 0 || lineCount !== undefined) {
        // Get total lines
        const { stdout: lineCountStr } = await this.executeCommand(
          executionId,
          `wc -l < "${containerPath}"`,
        );
        ...
        const endLine =
          lineCount !== undefined ? Math.min(startLine + lineCount, totalLines) : totalLines;
  ...
  async writeFile(executionId: string, filepath: string, content: string): Promise<void> {
    try {
      ...
      if (!this.isPathWithinWorkingDir(filepath, containerInfo)) {
        throw new Error(
          `Security constraint: Can only write to paths within the working directory. Attempted to write to ${filepath}`,
        );
      }
      ...
      if (content.length > maxChunkSize) {
        ...
        await this.executeCommand(
          executionId,
          `cat > "${tempFilePath}" << 'EOF_QCKFX'\n${content}\nEOF_QCKFX`,
        );
      }

      // Verify temp file was written correctly
      const { stdout: verifySize } = await this.executeCommand(
        executionId,
        `stat -c %s "${tempFilePath}"`,
      );
      const tempFileSize = parseInt(verifySize.trim(), 10);

      if (isNaN(tempFileSize) || tempFileSize === 0) {
        throw new Error(
          `Failed to write temporary file: file size verification failed (size: ${tempFileSize})`,
        );
      }
      ...
      await this.executeCommand(
        executionId,
        `cp "${tempFilePath}" "${containerPath}" && rm "${tempFilePath}"`,
      );

      // Verify destination file was written correctly
      const { stdout: finalSize } = await this.executeCommand(
        executionId,
        `stat -c %s "${containerPath}"`,
      );
      ...
      if (isNaN(finalFileSize) || finalFileSize === 0) {
        throw new Error(
          `Failed to write file: file size verification failed (size: ${finalFileSize})`,
        );
      }
  ...
  async editFile(
    ...
    try {
      ...
      if (!this.isPathWithinWorkingDir(containerPath, containerInfo)) {
        console.error(
          `Security constraint: Can only modify files within the working directory. Attempted to modify ${filepath}`,
        );
        return {
          ok: false as const,
      ...
      try {
        ...
        await this.executeCommand(
          executionId,
          `echo '${Buffer.from(normalizedSearchCode).toString('base64')}' | base64 -d > "${searchFile}"`,
        );
        await this.executeCommand(
          executionId,
          `echo '${Buffer.from(normalizedReplaceCode).toString('base64')}' | base64 -d > "${replaceFile}"`,
        );
        ...
        await this.executeCommand(
          executionId,
          `tr -d '\\r' < "${originalFile}" > "${originalFile}.unix" && mv "${originalFile}.unix" "${originalFile}"`,
        );
        ...
        const { exitCode: scriptExists } = await this.executeCommand(
          executionId,
          `which binary-replace.sh`,
        );
        if (scriptExists !== 0) {
        ...
        console.info(
          `Executing: binary-replace.sh "${originalFile}" "${searchFile}" "${replaceFile}" "${newFile}"`,
        );
        ...
        try {
          ...
          const { exitCode: origExists } = await this.executeCommand(
            executionId,
            `[ -f "${originalFile}" ]`,
          );
          const { exitCode: searchExists } = await this.executeCommand(
            executionId,
            `[ -f "${searchFile}" ]`,
          );
          const { exitCode: replaceExists } = await this.executeCommand(
            executionId,
            `[ -f "${replaceFile}" ]`,
          );
          ...
          const { exitCode: writeCheck, stderr: writeCheckErr } = await this.executeCommand(
            executionId,
            `touch "${containerPath}.writecheck" && rm "${containerPath}.writecheck"`,
          );
          ...
          const { exitCode: outputExists } = await this.executeCommand(
            executionId,
            `[ -f "${newFile}" ] && echo "Output file exists" || echo "Output file missing"`,
          );
        } catch (execError) {
          console.error(
            `[ERROR] Failed to execute binary-replace.sh: ${(execError as Error).message}`,
          );
        ...
        const { exitCode: newFileExists } = await this.executeCommand(
          executionId,
          `[ -f "${newFile}" ] && [ -s "${newFile}" ]`,
        );
        if (newFileExists !== 0) {
      ...
      return {
        ...
        data: {
          ...
          newContent: newFileResult.ok
            ? newFileResult.data.content
            : 'Content updated but unavailable for display', // Show the new content with line numbers
        },
      };
    } catch (error) {
      return {
  ...
  async glob(executionId: string, pattern: string, _options?: any): Promise<string[]> {
    try {
      ...
      const { stdout, exitCode } = await this.executeCommand(
        executionId,
        `find ${containerInfo.workspacePath} -path "${containerPattern}" -type f | sort`,
      );
  ...
  async ls(
    ...
    try {
      ...
      const {
        ...
        stderr,
        exitCode: lsExitCode,
      } = await this.executeCommand(executionId, command);
      ...
      if (details) {
        ...
        this.logger?.debug(
          `Parsed ${results.length} entries from ls -la output`,
          LogCategory.TOOLS,
        );
      } else {
        ...
        const { stdout: detailedOutput } = await this.executeCommand(
          executionId,
          `ls -la "${containerPath}"`,
        );
  ...
  private toContainerPath(hostPath: string, containerInfo: ContainerInfo): string {
    ...
    if (
      hostPath === containerInfo.workspacePath ||
  ...
  async generateDirectoryMap(rootPath: string, maxDepth: number = 10): Promise<string> {
    try {
      this.logger?.debug(
        `DockerExecutionAdapter: Generating directory map for ${rootPath} with max depth ${maxDepth}`,
        LogCategory.SYSTEM,
      );
      ...
      this.logger?.debug(
        `DockerExecutionAdapter: Directory map generated successfully: ${result.stdout}`,
        LogCategory.SYSTEM,
      );
      return result.stdout;
    } catch (error) {
      this.logger?.error(
        `DockerExecutionAdapter: Error generating directory map: ${(error as Error).message}`,
        LogCategory.SYSTEM,
      );
  ...
  async getGitRepositoryInfo(): Promise<GitRepositoryInfo[]> {
    try {
      ...
      if (batchResult.exitCode !== 0) {
        this.logger?.warn(
          `Batched git info command failed – falling back to individual exec: ${batchResult.stderr}`,
          LogCategory.SYSTEM,
        );
