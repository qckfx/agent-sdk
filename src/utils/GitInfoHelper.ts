  ...
  async getGitRepositoryInfo(
    ...
    try {
      ...
      if (!this.staticGitCache) {
        const isGitRepoCmd = await executeCommand(
        ...
        this.logger?.debug(
          `Static git info cached: defaultBranch=${defaultBranch}, gitDir=${gitDir}`,
          LogCategory.SYSTEM,
        );
      ...
      if (!this.staticGitCache.defaultBranch) {
        ...
        this.logger?.debug(
          `Default branch not found in remote, using: ${this.staticGitCache.defaultBranch}`,
          LogCategory.SYSTEM,
        );
      }
      ...
      if (!this.commitsCache || this.commitsCache.lastHeadCommit !== headCommit) {
        ...
        this.logger?.debug(
          `Updated commits cache with ${recentCommits.length} commits (HEAD: ${headCommit})`,
          LogCategory.SYSTEM,
        );
      } else {
