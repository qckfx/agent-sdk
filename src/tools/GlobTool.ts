  ...
  return createTool({
    ...
    parameters: {
      pattern: {
        type: 'string',
        description:
          "The glob pattern to match files. Examples: '**/*.js', 'src/**/*.json', '*.md'",
      },
      cwd: {
        type: 'string',
        description:
          "Base directory for the search. Use relative paths like 'src', '../', 'docs/v2' or absolute paths. Default: current directory ('.')",
      },
      dot: {
        type: 'boolean',
      ...
      try {
        ...
        if (isSandbox && path.isAbsolute(cwd)) {
          ...
          if (!cwd.startsWith(sandboxRoot)) {
            context.logger?.warn(
              `Warning: GlobTool: Using absolute path outside sandbox: ${cwd}. This may fail.`,
            );
          }
        }
        ...
        if (matches.length > MAX_RESULTS_SIZE) {
          context.logger?.info(
            `GlobTool: Truncating ${matches.length} results to ${MAX_RESULTS_SIZE} to avoid message size limits`,
          );
