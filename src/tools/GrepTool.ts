  ...
  return createTool({
    ...
    parameters: {
      pattern: {
        type: 'string',
        description:
          'Search pattern to look for in files. Can be plain text or regular expressions.',
      },
      path: {
        type: 'string',
        description:
          "Directory or file path to search in. Can be relative like 'src', '../' or absolute. Default: current directory",
      },
      recursive: {
        type: 'boolean',
      ...
      filePattern: {
        type: 'string',
        description:
          "Optional glob pattern to filter which files to search. Example: '*.js', '*.{js,ts}'",
      },
      maxResults: {
        type: 'number',
    ...
    execute: async (
      ...
      try {
        ...
        const { stdout } = await context.executionAdapter.executeCommand(
          context.executionId,
          command,
        );
        ...
        let results: GrepResult[] = lines
          ...
          .map(line => {
            ...
            if (match) {
              ...
              const relativePath =
                filePath.startsWith(searchPath) && searchPath !== '.'
                  ? filePath.substring(searchPath.length + 1)
                  : filePath;
        ...
        if (results.length > MAX_RESULTS_SIZE) {
          context.logger?.info(
            `GrepTool: Truncating ${results.length} results to ${MAX_RESULTS_SIZE} to avoid message size limits`,
          );
