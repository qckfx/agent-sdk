  ...
  return createTool({
    ...
    parameters: {
      path: {
        type: 'string',
        description:
          "The directory to list contents from. Use relative paths like 'src', '../', 'docs/v2' or absolute paths. Default: current directory ('.')",
      },
      showHidden: {
        type: 'boolean',
    ...
    execute: async (args: LSToolArgs, context: ToolContext): Promise<LSToolResult> => {
      ...
      if (isSandbox && path.isAbsolute(dirPath)) {
        ...
        if (!dirPath.startsWith(sandboxRoot)) {
          context.logger?.warn(
            `Warning: LSTool: Using absolute path outside sandbox: ${dirPath}. This may fail.`,
          );
        }
