...
const createPermissionManager = (
  ...
  return {
    ...
    async requestPermission(
      ...
      if (dangerMode) {
        logger?.info(
          `DANGER_MODE enabled, auto-approving tool: ${toolId}`,
      ...
      if (!tool) {
        logger?.info(
          `Unknown tool ${toolId}, requesting permission from user`,
          LogCategory.PERMISSIONS,
        );
      ...
      if (tool.alwaysRequirePermission) {
        logger?.info(
          `Tool ${toolId} has alwaysRequirePermission=true, requesting permission`,
          LogCategory.PERMISSIONS,
        );
        return await uiHandler.requestPermission(sessionId, toolId, args);
      }
      ...
      if (fastEditMode && toolRegistry.isToolInCategory(toolId, ToolCategory.FILE_OPERATION)) {
        logger?.info(
          `Fast Edit Mode enabled, auto-approving file operation: ${toolId}`,
          LogCategory.PERMISSIONS,
        );
        return true;
      }
      ...
      if (!tool.requiresPermission) {
        logger?.info(
          `Tool ${toolId} has requiresPermission=false, auto-approving`,
          LogCategory.PERMISSIONS,
        );
        return true;
      }
    ...
    enableDangerMode(): void {
      dangerMode = true;
      logger?.warn(
        'DANGER_MODE enabled - all tools will be auto-approved',
        LogCategory.PERMISSIONS,
      );
    },
