...
describe('Agent', () => {
  ...
  it('should use defaultModel if no model is provided to processQuery', async () => {
    ...
    expect(processQuery).toHaveBeenCalledWith('test query', 'test-model', expect.anything());
  });
  ...
  it('should use provided model over defaultModel in processQuery', async () => {
    ...
    expect(processQuery).toHaveBeenCalledWith('test query', 'override-model', expect.anything());
  });
