/**
 * TextBuffer implementation (adapted from the openai/codex open-source project).
 *
 ...
 *
 * Modifications Copyright (c) 2025 qckfx. All additional modifications are
 * licensed under the project's MIT license, but the Apache-2.0 terms above
 * still apply to the portions coming from the original work.
 */
...
function toCodePoints(str: string): Array<string> {
  if (typeof Intl !== 'undefined' && 'Segmenter' in Intl) {
    ...
    return [...seg.segment(str)].map((seg: any) => seg.segment);
  }
...
const DEBUG = process.env['TEXTBUFFER_DEBUG'] === '1' || process.env['TEXTBUFFER_DEBUG'] === 'true';
...
export default class TextBuffer {
   ...
   * @param state
   */
  private restore(state: { lines: Array<string>; row: number; col: number } | undefined): boolean {
    if (!state) {
      return false;
    }
  ...
  deleteWordLeft(): void {
    ...
    this.lines[this.cursorRow] = cpSlice(line, 0, start) + cpSlice(line, this.cursorCol);
  ...
  deleteWordRight(): void {
    ...
    const line = this.line(this.cursorRow);
    const arr = toCodePoints(line);
    if (this.cursorCol >= arr.length && this.cursorRow === this.lines.length - 1) {
      return;
    } // nothing to delete
    ...
    this.lines[this.cursorRow] = cpSlice(line, 0, this.cursorCol) + cpSlice(line, end);
    ...
    switch (dir) {
      ...
      case 'up':
        if (this.cursorRow > 0) {
          ...
          this.cursorRow--;
          this.cursorCol = clamp(this.preferredCol, 0, this.lineLen(this.cursorRow));
        }
        break;
      case 'down':
      ...
      case 'wordLeft': {
        this.preferredCol = null;
        const regex = /[\s,.;!?]+/g;
        const slice = cpSlice(this.line(this.cursorRow), 0, this.cursorCol).replace(
          /[\s,.;!?]+$/,
          '',
        );
        let lastIdx = 0;
  ...
  handleInput(input: string | undefined, key: Record<string, boolean>, vp: Viewport): boolean {
    ...
    } else if (key['backspace'] || input === '\x7f' || (key['delete'] && !key['shift'])) {
    ...
    this.ensureCursorInRange();
    this.ensureCursorVisible(vp);
    const cursorMoved = this.cursorRow !== beforeRow || this.cursorCol !== beforeCol;
